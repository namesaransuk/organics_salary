...
class WithdrawSavingsViewScreen extends StatefulWidget {
  const WithdrawSavingsViewScreen({super.key});

  @override
  State<WithdrawSavingsViewScreen> createState() =>
      _WithdrawSavingsViewScreenState();
}

class _WithdrawSavingsViewScreenState extends State<WithdrawSavingsViewScreen> {
  ...
  void initState() {
    ...
    savingsController.endMonthYearName.value =
        DateFormat('yyyy-MM').format(today).toString();
    savingsController.loadDataWithdraw();
  ...
  List<Savings> getEmployeeData() {
    return savingsController.savingsWithdrawList.map((savings) {
      ...
      String formattedChannel =
          savings.saveChannel == 1 ? 'โอนจากบัญชี' : 'อื่นๆ';
      return Savings(
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Obx(
          () => savingsController.savingsWithdrawList.isNotEmpty
              ? Expanded(
